...
dungeon_map_roof = layer_tilemap_get_id("Roof")
...
repeat(steps) {
	...
	if (drunkard_x < 16 || drunkard_x >= ds_grid_width(dungeon_grid)-17) 
		drunkard_x += - drunkard_direction_x * 2;
		
	
	if (drunkard_y < 16 || drunkard_y >= ds_grid_height(dungeon_grid)-17)
...
for (i = 1 ; i < ds_grid_height(dungeon_grid) - 1 ; i++ ) {
	for (j = 1 ; j < ds_grid_width(dungeon_grid) - 1 ; j++ ) {
		show_debug_message(ds_grid_get(dungeon_grid, j,i));
		if (ds_grid_get(dungeon_grid, j, i) != "Floor") {
			...
			instance_create_layer(j*16+4, i*16+8, "Instances",  objWall);
		}
	}
}
...
for (i = 1 ; i < ds_grid_height(dungeon_grid) - 1 ; i++ ) {
	for (j = 1 ; j < ds_grid_width(dungeon_grid) - 1 ; j++ ) {
		...
		if (ds_grid_get(dungeon_grid, j, i) != "Floor") {
			...
			tile_index = NORTH * tile_north + WEST * tile_west + EAST * tile_east + SOUTH * tile_south + 1;	
			tilemap_set(dungeon_map_roof, tile_index, j, i);
...
while (player_set == false) {
	...
	if ds_grid_get(dungeon_grid, player_x, player_y) == "Floor" {
		instance_create_layer(player_x*16 + sprite_get_width(sprPlayerDungeonIdle)/2, player_y*16 + sprite_get_height(sprPlayerDungeonIdle)/2, "Instances",  DungeonPlayer);
		player_set = true;
	}
}